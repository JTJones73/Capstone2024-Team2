...
* C21: Time to move launcher into aiming position must be less than 0.19 seconds.
...
For C21, DEVCOM gave the team a set of drop times from the game board that they developed. The fastest drop time recorded was 1.98 s on line 8 height 2. Assuming that the target has a potential to travel 15 percent faster at a worst-case drop time of 1.78 s, it could be determined that the projectile path sensor if placed at the first sensor post, the time it would take for the path of the target would be around 0.91 s. It will also take 1.27 s to reach the longest point of interception of 28 inches from the back of the blue square in the arena layout, leaving 0.36 s to process, aim the launcher, and launch the projectile to intercept. Given that it will take 0.08 s to launch the projectile and 0.078 s for the launcher to intercept a projectile from 28 inches away at 30 feet per second, and the maximum processing time is estimated to take 0.02 s, it leaves 0.19 s as the maximum time to move the launcher to the target.
